...
class SimConfiguration(object):
    ...
    def __init__(self,
        ...
        if configs is not None:
            ...
            self.auto_ms_bs_calc = self.configs['transformation_temp_limits'][
                'auto_calculate']
            self.ms_temp = self.configs['transformation_temp_limits'][
                'ms_temp']
            self.ms_undercool = self.configs['transformation_temp_limits'][
                'ms_undercool']
            self.bs_temp = self.configs['transformation_temp_limits'][
                'bs_temp']
    ...
    def _get_default_test_configs(self) -> dict:
        ...
        self.ms_undercool = config['transformation_temp_limits'][
            'ms_undercool']
    ...
    def get_bs(self) -> float:
        ...
        mn = self.comp_parent[self.comp_parent['name'] ==
                              'manganese']['weight'][0]
        ni = self.comp_parent[self.comp_parent['name'] ==
                              'nickel']['weight'][0]
        cr = self.comp_parent[self.comp_parent['name'] ==
                              'chromium']['weight'][0]
        mo = self.comp_parent[self.comp_parent['name'] ==
                              'molybdenum']['weight'][0]

        if self.method == Method.Li98:
            # Eqn [24] in paper. Li modified from Kirkaldy.
            return 637.0 - (58 * c) - (35 * mn) - (15 * ni) - (34 * cr) - (41 *
                                                                           mo)

        if self.method == Method.Kirkaldy83:
            # Eqn [30] in Kirkaldy defined 1983 paper
            return 656 - (58 * c) - (35 * mn) - (15 * ni) - (34 * cr) - (41 *
                                                                         mo)
        return -1
    ...
    def get_ms(self) -> float:
        ...
        mo = self.comp_parent[self.comp_parent['name'] ==
                              'molybdenum']['weight'][0]
        co = self.comp_parent[self.comp_parent['name'] ==
                              'cobalt']['weight'][0]
        si = self.comp_parent[self.comp_parent['name'] ==
                              'silicon']['weight'][0]
        ...
        if self.method == Method.Kirkaldy83:
            ...
            return 561 - (474 * c) - (33.0 * mn) - (17.0 * ni) - (
                17.0 * cr) - (21.0 * mo)
    ...
    def calc_ae1_ae3(self) -> (np.float, np.float):
        ...
        si = self.comp_parent[self.comp_parent['name'] ==
                              'silicon']['weight'][0]
        wx = self.comp_parent[self.comp_parent['name'] ==
                              'tungsten']['weight'][0]
        mn = self.comp_parent[self.comp_parent['name'] ==
                              'manganese']['weight'][0]
        cr = self.comp_parent[self.comp_parent['name'] ==
                              'chromium']['weight'][0]
        asx = self.comp_parent[self.comp_parent['name'] ==
                               'arsenic']['weight'][0]
        mo = self.comp_parent[self.comp_parent['name'] ==
                              'molybdenum']['weight'][0]
        vx = self.comp_parent[self.comp_parent['name'] ==
                              'vanadium']['weight'][0]
        cu = self.comp_parent[self.comp_parent['name'] ==
                              'copper']['weight'][0]
        px = self.comp_parent[self.comp_parent['name'] ==
                              'phosphorous']['weight'][0]
        al = self.comp_parent[self.comp_parent['name'] ==
                              'aluminium']['weight'][0]
        ti = self.comp_parent[self.comp_parent['name'] ==
                              'titanium']['weight'][0]
        # Do the calculations
        # 1. Equations of Andrews (1965)
        ae1 = (723.0 - (16.9 * ni) + (29.1 * si) + (6.38 * wx) - (10.7 * mn) +
               (16.9 * cr) + (290 * asx)) / 3.0

        # 2. Equations of Eldis (in Barralis, 1982): 1/3 due to averaging
        ae1 = ae1 + (712.0 - (17.8 * mn) - (19.1 * ni) + (20.1 * si) +
                     (11.9 * cr) + (9.8 * mo)) / 3.

        # 3. Equations of Grange (1961): 1/3 due to averaging, convert from F to C
        # (-32*(5/9))
        ae1 = ae1 + (1333.0 - (25.0 * mn) + (40.0 * si) + (42.0 * cr) -
                     (26.0 * ni) - 32.0) * 5.0 / (3.0 * 9.0)
...
{}
-------------------------------------------------
        """.format('Method:', self.method.name, 'Alloy:', self.alloy.name,
                   'Nucleation Start:', self.nuc_start, 'Nucleation End:',
                   self.nuc_finish, 'Type:', self.grain_type, 'Value: ',
                   self.grain_size, 'Auto Calculate:', self.auto_xfe_calc,
                   'Xfe:', self.xfe, 'Cf:', self.cf, 'Ceut:', self.ceut,
                   'Auto Calculate:', self.auto_ms_bs_calc, 'MS Temperature:',
                   self.ms_temp, 'MS Undercool: ', self.ms_undercool,
                   'BS Temperature: ', self.bs_temp, 'Auto Calculate:',
                   self.auto_austenite_calc, 'Ae1:', self.ae1, 'Ae3:',
                   self.ae3, parent_t, weld_t, mix_t)

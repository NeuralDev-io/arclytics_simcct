...
from users_app.models import (
    User, UserProfile, AdminProfile, Configuration, SharedSimulation,
    AlloyStore
)
...
class ShareSimulationLink(Resource):
    ...
    def post(self, resp) -> Tuple[dict, int]:
        ...
        config_signature = generate_shared_simulation_signature(
            shared_config_object.to_dict()
        )
        config_url = generate_url_with_signature(
            'users.request_shared_simulation', config_signature
        )
...
@users_blueprint.route('/users/share/simulation/request/<signature>',
                       methods=['GET']
                       )
...
@users_blueprint.route('/users/share/simulation/view/<signature>',
                       methods=['GET']
                       )

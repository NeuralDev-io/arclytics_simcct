...
class PhaseSimulation(object):
    ...
    def ttt(self) -> dict:
        ...
        fcf_mat = np.zeros((10001, 2),
                           dtype=np.float64)  # Ferrite Curve Finish

        for i in range(1, 3):
            temp_curr = self.bs - 50
            count_fn = 0
            while temp_curr < (self.ae3 - 1):
                torr = self.__torr_calc2(torr, Phase.F, temp_curr,
                                         integrated2_mat, i)
        ...
        # ========= PEARLITE PHASE ========= #
        pcs_mat = np.zeros((10001, 2),
                           dtype=np.float64)  # Pearlite curve start
        pcf_mat = np.zeros((10001, 2),
                           dtype=np.float64)  # Pearlite curve finish
        ...
        for i in range(1, 3):
            ...
            while temp_curr < (self.ae1 - 1):
                torr = self.__torr_calc2(torr, Phase.P, temp_curr,
                                         integrated2_mat, i)
                if i is 1:
        ...
        bcf_mat = np.zeros((10001, 2),
                           dtype=np.float64)  # Bainite curve finish
        ...
        for i in range(1, 3):
            ...
            while temp_curr < (self.bs - 1):
                torr = self.__torr_calc2(torr, Phase.B, temp_curr,
                                         integrated2_mat, i)
        ...
        self.plots_data.set_ttt_plot_data(ferrite_start=fcs_mat,
    ...
    def cct(self) -> dict:
        ...
        cct_record_f_end_mat = np.zeros((10000, 2),
                                        dtype=np.float64)  # Ferrite
        cct_record_p_end_mat = np.zeros((10000, 2),
                                        dtype=np.float64)  # Pearlite
        cct_record_b_end_mat = np.zeros((10000, 2),
                                        dtype=np.float64)  # Bainite
        ...
        self.__critical_cooling_rate(ccr_mat, self.ms, self.bs, self.ae1,
                                     self.ae3, integrated2_mat)
        ...
        for i in range(100):
            ...
            while temp_curr > self.ae3:
                ...
                temp_curr = self.__next_temp(temp_curr, cooling_rate,
                                             increm_time)
    ...
    def __sigmoid2(x) -> np.float64:
        return np.float64(1 / (math.pow(x, (0.4 * (1 - x))) * math.pow(
            (1 - x), (0.4 * x))))

    @staticmethod
    def __imoid(x) -> np.float64:
        """Kirkaldy 1983 method I(X)."""
        return np.float64(1 / (math.pow(x, (2.0 * (1 - x) / 3)) * math.pow(
            (1 - x), (2.0 * x / 3))))
    ...
    def __imoid_prime2(self, x: float) -> np.float64:
        ...
        numerator = np.float64(
            math.exp(
                math.pow(x, 2) * ((1.9 * c) + (2.5 * mn) + (0.9 * ni) +
                                  (1.7 * cr) + (4.0 * mo) - 2.6)))

        b = np.float64(((1.9 * c) + (2.5 * mn) + (0.9 * ni) + (1.7 * cr) +
                        (4.0 * mo) - 2.6))

        if b < 0:
            return np.float64(1 / (math.pow(x,
                                            (2.0 * (1 - x) / 3.0)) * math.pow(
                                                (1 - x), (2.0 * x / 3.0))))

        return np.float64(numerator /
                          (math.pow(x, (2.0 * (1 - x) / 3.0)) * math.pow(
                              (1 - x), (2.0 * x / 3.0))))
    ...
    def __vol_phantom_frac2(self, integrated_mat) -> None:
        ...
        if self.configs.method == Method.Li98:
            ...
            sig_int_ferrite = 0
            sig_int_ferrite = self.__de_integrator(sig_int_ferrite, 0.0, xf,
                                                   0.000000000000001, err, nn,
                                                   1)
            ...
            sig_int_pearlite = self.__de_integrator(sig_int_pearlite, 0.0, xp,
                                                    0.000000000000001, err, nn,
                                                    1)
            ...
            sig_int_bainite = 0
            sig_int_bainite = self.__de_integrator(sig_int_bainite, 0.0, xb,
                                                   0.000000000000001, err, nn,
                                                   1)
    ...
    def __torr_calc2(self, torr: float, phase: Phase, tcurr: float,
                     integral2_mat: np.ndarray, i: int) -> np.float64:
        R_GAS = 1.985
        ...
        if self.configs.method == Method.Li98:
            ...
            if phase == Phase.F:
                fc = np.float64(
                    math.exp(1.0 + (6.31 * c) + (1.78 * mn) + (0.31 * si) +
                             (1.12 * ni) + (2.7 * cr) + (4.06 * mo)))

                return np.float64(
                    fc / (math.pow(2, (0.41 * self.g)) * math.pow(
                        (self.ae3 - tcurr), 3) * math.exp(-27500 /
                                                          (R_GAS *
                                                           (tcurr + 273)))) *
                    sint_f)
                ...
                pc = np.float64(
                    math.exp(-4.25 + (4.12 * c) + (4.36 * mn) + (0.44 * si) +
                             (1.71 * ni) + (3.33 * cr) +
                             (5.19 * math.sqrt(mo))))

                return np.float64(
                    pc / (math.pow(2, (0.32 * self.g)) * math.pow(
                        (self.ae1 - tcurr), 3) * math.exp(-27500 /
                                                          (R_GAS *
                                                           (tcurr + 273)))) *
                    sint_p)
                ...
                bc = np.float64(
                    math.exp(-10.23 + (10.18 * c) + (0.85 * mn) + (0.55 * ni) +
                             (0.9 * cr) + (0.36 * mo)))

                return np.float64(
                    bc / (math.pow(2, (0.29 * self.g)) * math.pow(
                        (self.bs - tcurr), 2) * math.exp(-27500 /
                                                         (R_GAS *
                                                          (tcurr + 273)))) *
                    sint_b)
            ...
            if phase == Phase.F:
                ...
                return np.float64(
                    fc /
                    (2**((self.g - 1) / 2) *
                     ((self.ae3 - tcurr)**3) * math.exp(-23500 /
                                                        (R_GAS *
                                                         (tcurr + 273)))) *
                    iint_f)
                ...
                dinv = np.float64((1 / math.exp(-27500 / (R_GAS *
                                                          (tcurr + 273)))) +
                                  ((0.01 * cr + 0.52 * mo) /
                                   math.exp(-37500 / (R_GAS * (tcurr + 273)))))

                return np.float64(
                    pc / ((2**((self.g - 1) / 2) * ((self.ae1 - tcurr)**3) *
                           (1 / dinv)) * iint_p))

            elif phase == Phase.B or phase == Phase.M:
                bc = np.float64((2.34 + (10.1 * c) + (3.8 * cr) +
                                 (19.0 * mo)) * math.pow(10, -4))
    ...
    def __critical_cooling_rate(self, ccr_mat: np.ndarray, ms: float,
                                bs: float, ae1: float, ae3: float,
                                integrated2: np.ndarray):
        """
        ...
        ccr_mat[0, 1] = ccr_fcf / ((ae3 - bs) /
                                   (time_accumulate - time_interval))

    @staticmethod
    def __next_temp(current_temp: float, cooling_rate: float,
                    time_inc_sec: float) -> np.float64:
    ...
    def test_critical_cooling_rate(self, ccr_mat: np.ndarray, ms: float,
                                   bs: float, ae1: float, ae3: float,
                                   integrated2: np.ndarray):
        self.__critical_cooling_rate(ccr_mat, ms, bs, ae1, ae3, integrated2)

    def test_torr_calc2(self, torr: float, phase: Phase, tcurr: float,
                        integral2_mat: np.ndarray, i: int) -> np.float64:
